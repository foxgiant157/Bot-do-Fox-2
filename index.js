	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo la tropinha *${mdata.subject}*\nPor favor não fica calado nessa praga❤️`
				...
				teks = `Tchau @${num.split('@')[0]} não faz diferença 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'sticker':
					...
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Qual é o código da linguagem doente', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('Texto grande do caraca em vey')
				...
				case 'meme':
					...
					buffer = await getBuffer(`http://imgur.com/gallery/GKolvga`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
				case 'porno':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`http://imgur.com/gallery/0HONMb8`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Hzin gostosa'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'loli1':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`http://imgur.com/gallery/jPhM7HM`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'lolizinha pra la tropa'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`http://imgur.com/gallery/OCIT4xa`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aproveita, vc n vai pro inferno'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/7VL9cFf.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, la tropinha sempre!❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde, la tropinha reina!😎👍'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
				...
				case 'malkova':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://photos.google.com/share/AF1QipPPsltlFGiSwrKt9H_btsUBEzqGdPua44MTkOl7U2cZjbdlfTzfFo27eVU7-S0Cgg/photo/AF1QipOHAvqmpzRXKzEGmVcVz_6_DOV_5b4JFloOBSee`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️💆'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'La sirena':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`http://imgur.com/a/c3E257S`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '🤭'})
					break
				case 'mia2':
					memein = await kagApi.memeindo()
				...
				case 'memeindo':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`http://imgur.com/gallery/LyBQkPu`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
				case 'setprefix':
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} virou ademiro, então respeita o brabo 😎👍`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} Não merece ser ademiro 😂!`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Banido otario, respeita a la tropinha 🙊 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
